...
Also, install `biopython` and `polyleven` before running this code: `pip install biopython polyleven` (it's not included in the requirements.txt)
Make sure that git-lfs is installed in your system before cloning the repository, as the data files are stored using git-lfs.
'''
...
class AMPRewardProxyDataset(RewardProxyDataset):
    '''
    This class is a dataset for the AMP reward proxy task, needed to train a reward proxy model
    Credits: https://github.com/MJ10/BioSeq-GFN-AL/blob/faster-impl/lib/dataset/amp.py
    '''
    char_to_id = {char: i for i, char in enumerate(AMINO_ACIDS + SPECIAL_TOKENS)}
    ...
    def __init__(self, split='D', nfold=5) -> None:   
        ...
        groups = np.concatenate((np.array(source.d1_pos.group), np.array(source.d2_pos.group)))
        n_pos, n_neg = len(data['AMP']), len(data['nonAMP'])
        pos_train, pos_valid = next(GroupKFold(nfold).split(np.arange(n_pos), groups=groups))
        neg_train, neg_valid = next(GroupKFold(nfold).split(np.arange(n_neg), groups=rng.randint(0, nfold, n_neg)))
        ...
        self.train_score = jnp.concatenate([jnp.ones(len(pos_train)), jnp.zeros(len(neg_train))])
        self.test_score = jnp.concatenate([jnp.ones(len(pos_valid)), jnp.zeros(len(neg_valid))])
