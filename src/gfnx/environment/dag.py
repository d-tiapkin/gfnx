...
class DAGEnvironment(BaseVecEnvironment[EnvState, EnvParams]):
    ...
    def get_invalid_backward_mask(
        ...
        """
        # Step 1: Invert adjacency matrix
        num_envs = state.time.shape[0]
        mask = jnp.logical_not(state.adjacency_matrix)  # invert mask
        mask = jnp.logical_or(
            jnp.tile(
                jnp.eye(self.num_variables, dtype=jnp.bool), (num_envs, 1, 1)
            ),
            mask,
        )  # cannot remove self-loops
        mask = mask.reshape(num_envs, -1)  # flatten
        # Step 2: Allow stop action only for the terminal state
        fwd_mask = jnp.logical_or(
            state.adjacency_matrix, state.closure_T
        ).reshape(num_envs, -1)
        stop_mask = jnp.logical_not(jnp.all(fwd_mask, axis=-1, keepdims=True))
        mask = jnp.concatenate([mask, stop_mask], axis=1)
        return mask
